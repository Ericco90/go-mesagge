...
def parse_text(text, kabupaten_name):
    data = []
    current_kec = None
    current_lat = None
    current_lon = None
    ...
    for line in lines:
        ...
        # Match kecamatan line, e.g., "**1. Kecamatan Margasari (-7.0977, 109.0215) — 13 desa:**"
        m = re.match(r'\*\*\d+\.\s+Kecamatan\s+([A-Za-z\s]+)\s+\(([-\d\.]+),\s+([-\d\.]+)\)', line)
        if m:
            current_kec = m.group(1).strip()
            current_lat = float(m.group(2))
            current_lon = float(m.group(3))
            continue
        
        # Match desa line, e.g., "- Danaraja"
        if line.startswith('- ') and current_kec is not None:
            desa_name = line[2:].strip()
            # Remove "(desa)" or "(kelurahan)"
            desa_name = re.sub(r'\s*\((desa|kelurahan)\)', '', desa_name, flags=re.IGNORECASE).strip()
            # Remove asterisks if any
            desa_name = desa_name.replace('*', '')
            
            data.append({
                'name': f"{desa_name}, {current_kec}, {kabupaten_name}",
                'lat': current_lat,
                'lon': current_lon
            })
    return data

tegal_data = parse_text(tegal_text, "Kabupaten Tegal")
brebes_data = parse_text(brebes_text, "Kabupaten Brebes")

all_data = tegal_data + brebes_data

js_content = f"const DESA_DATABASE = {json.dumps(all_data, indent=4)};\n"

with open("c:\\Users\\USER\\Desktop\\jastip\\desa-data.js", "w") as f:
    f.write(js_content)

print(f"Generated {len(all_data)} desa entries.")
